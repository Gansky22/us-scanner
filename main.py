...
def score_t_stock(symbol, daily_df, intraday_df=None, market_mode="NEUTRAL"):
    """V21.5 做T评分：严格A级过滤 + RVOL/VWAP/ORB强制降级 + 双止损。"""
...
def build_t_radar_text(title, arr):
    ...
    lines.append("V21.5做T规则：A级必须量能+VWAP+趋势合格；B级只轻仓；C级只观察；D级不做。做T止损是日内纪律，波段止损是趋势底线。")
    return "\n".join(lines)
...
def run_t_radar(force=False):
    ...
    save_state()
    send_telegram(build_t_radar_text("⚡ V21.5 严格做T雷达 Top10", top))
...
@app.route("/")
def home():
    return "V21.5 Strict T-Radar + Smart Money Pre-Breakout Live Trading Monitor Running"

@app.route("/health")
def health():
    load_watchlist()
    return jsonify({"status": "healthy", "time_kl": now_kl_str(), "symbols": len(ALL_SYMBOLS), "my_stocks": MY_STOCKS, "positions_count": len(load_positions()), "version": "V21.5_strict_t_radar_dual_stop_orb"})
...
def route_test():
    ok = send_telegram("✅ V21.5 Strict T-Radar Telegram Test Success")
