...
# ============================================================
# 美股爆发扫描器 V23.2 专业做T系统 + 事件风控 + 动态热点池 + 交易数据库版
...
HOT_POOL_SCAN_LIMIT = int(os.getenv("HOT_POOL_SCAN_LIMIT", "120"))             # 热点池最多扫描多少支，避免超时
...
def scan_hot_pool():
    ...
    if not HOT_POOL_ENABLED:
        return []
    results = []
    symbols = [s for s in ALL_SYMBOLS if s not in BAD_SYMBOLS and not should_skip_symbol(s)]
    if HOT_POOL_SCAN_LIMIT > 0:
        symbols = symbols[:HOT_POOL_SCAN_LIMIT]
...
def run_hot_pool(force=False):
    ...
    lines = ["🔥 V23.2 今日自动热点池", now_kl_str(), ""]
...
@app.route("/")
def home():
    return "V23.2 Professional T-Radar + Event Risk + Hot Pool + Trade Journal Running"
...
def route_system_status():
    return jsonify({
        "version": "V23.2_stable_professional_t_radar",
        "time_kl": now_kl_str(),
