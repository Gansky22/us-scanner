...
# V23.2 新增：交易数据库、模式统计、胜率/盈亏总结、系统化复盘
# V23.2 新增：稳定优化、Yahoo错误静默、坏数据冷却、热点池限速、防卡死
# ============================================================
...
DEFAULT_MY_STOCKS = os.getenv("MY_STOCKS", "LUNR,IONQ,SMCI,AMD,CRM,TSLA,META,SOFI,PLTR,TEM,AMZN,APP,ONDS,SOUN,NVDA,AFRM").strip()
...
# V24 双向ETF T 模块：只提醒，不自动下单
ETF_T_STOCKS_RAW = os.getenv("ETF_T_STOCKS", "QQQ,SPY,TQQQ,SQQQ,SOXL,SOXS").strip()
...
HOT_POOL_MIN_RVOL = float(os.getenv("HOT_POOL_MIN_RVOL", "0.8"))
HOT_POOL_MIN_RANGE = float(os.getenv("HOT_POOL_MIN_RANGE", "1.5"))
HOT_POOL_MIN_DOLLAR_VOLUME = float(os.getenv("HOT_POOL_MIN_DOLLAR_VOLUME", "30000000"))
...
# V23.2 稳定优化参数
# 目的：避免 yfinance 个别股票无数据导致红字/卡顿/重复请求
YF_SUPPRESS_ERRORS = os.getenv("YF_SUPPRESS_ERRORS", "1") == "1"               # 静默yfinance红字
YF_FAIL_LIMIT = int(os.getenv("YF_FAIL_LIMIT", "2"))                           # 同一股票失败几次后临时跳过
YF_FAIL_SKIP_MINUTES = int(os.getenv("YF_FAIL_SKIP_MINUTES", "30"))             # 临时跳过时间
YF_DOWNLOAD_TIMEOUT = int(os.getenv("YF_DOWNLOAD_TIMEOUT", "12"))              # 单次下载超时秒数
HOT_POOL_SCAN_LIMIT = int(os.getenv("HOT_POOL_SCAN_LIMIT", "300"))             # 热点池最多扫描多少支，避免超时
SEND_SCAN_ERROR_TELEGRAM = os.getenv("SEND_SCAN_ERROR_TELEGRAM", "0") == "1"   # 默认不推送扫描错误
...
ETF_T_STOCKS = parse_symbols(ETF_T_STOCKS_RAW)
...
def should_skip_symbol(symbol):
    """同一股票连续抓取失败后，临时跳过，避免一直刷红字/拖慢扫描。"""
    symbol = clean_symbol(symbol)
    item = YF_FAIL_CACHE.get(symbol)
    if not item:
        return False
    fail_count = int(item.get("count", 0))
    last_ts = float(item.get("last_ts", 0))
    if fail_count < YF_FAIL_LIMIT:
        return False
    return (time.time() - last_ts) < (YF_FAIL_SKIP_MINUTES * 60)


def mark_symbol_success(symbol):
    symbol = clean_symbol(symbol)
    if symbol in YF_FAIL_CACHE:
        del YF_FAIL_CACHE[symbol]


def mark_symbol_fail(symbol, reason=""):
    symbol = clean_symbol(symbol)
    item = YF_FAIL_CACHE.get(symbol, {"count": 0, "last_ts": 0, "reason": ""})
    item["count"] = int(item.get("count", 0)) + 1
    item["last_ts"] = time.time()
    item["reason"] = str(reason)[:120]
    YF_FAIL_CACHE[symbol] = item


def stable_yf_download(symbol, **kwargs):
    """包装 yfinance：静默错误 + timeout + 失败冷却。"""
    symbol = clean_symbol(symbol)
    ...
    try:
        ...
        kwargs.setdefault("timeout", YF_DOWNLOAD_TIMEOUT)
        if YF_SUPPRESS_ERRORS:
            with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
                return yf.download(symbol, **kwargs)
        return yf.download(symbol, **kwargs)
    except Exception as e:
        mark_symbol_fail(symbol, type(e).__name__)
        return None


def valid_price_df(df, min_rows=3):
...
def safe_download(symbol, period="6mo", interval="1d"):
    symbol = clean_symbol(symbol)
    try:
        df = stable_yf_download(symbol, period=period, interval=interval, auto_adjust=True)
        min_rows = 30 if interval == "1d" else 3
        df = valid_price_df(df, min_rows=min_rows)
        if df is None:
            mark_symbol_fail(symbol, "empty_or_invalid_daily")
            return None
        mark_symbol_success(symbol)
        return df
    except Exception as e:
        mark_symbol_fail(symbol, type(e).__name__)
        return None
...
def safe_download_intraday(symbol, period="5d", interval="5m", prepost=True):
    """V23.2: 盘中/盘前数据，静默错误并跳过临时坏数据。"""
    symbol = clean_symbol(symbol)
    try:
        df = stable_yf_download(symbol, period=period, interval=interval, auto_adjust=True, prepost=prepost)
        df = valid_price_df(df, min_rows=3)
        if df is None:
            mark_symbol_fail(symbol, "empty_or_invalid_intraday")
            return None
        mark_symbol_success(symbol)
        return df
    except Exception as e:
...
def _score_hot_candidate_fallback(symbol):
    """V24: 当严格Hot Pool没有结果时，用较宽条件从自选股/ETF里找观察股，避免完全空白。"""
    symbol = clean_symbol(symbol)
    if symbol in BAD_SYMBOLS or should_skip_symbol(symbol):
        return None
    ...
    if dollar_volume < 10_000_000 and symbol not in MY_STOCKS:
        return None
    avg_range20 = float(((high - low) / close * 100).rolling(20).mean().iloc[-1])
    change1 = pct(last, float(close.iloc[-2]))
    hot_score = intraday_rvol * 15 + today_range * 2.5 + change1 * 2 + min(dollar_volume / 100000000, 8)
    return {
        "symbol": symbol,
        "hot_score": round(hot_score, 1),
        "effective_rvol": round(intraday_rvol, 2),
...
def scan_hot_pool():
    """V24: 自动热点池。优先扫MY_STOCKS，再扫ETF，再扫行业池，避免强股被sorted截断漏掉。"""
    if not HOT_POOL_ENABLED:
        return []
    load_watchlist()
    results = []
    symbols = list(dict.fromkeys(MY_STOCKS + ETF_T_STOCKS + ALL_SYMBOLS))
    symbols = [s for s in symbols if s not in BAD_SYMBOLS and not should_skip_symbol(s)]
    if HOT_POOL_SCAN_LIMIT > 0:
        # V24: 永远保留 MY_STOCKS + ETF_T_STOCKS，limit只限制后面的行业池
        priority = [s for s in list(dict.fromkeys(MY_STOCKS + ETF_T_STOCKS)) if s in symbols]
    ...
    for batch in chunk_list(symbols, BATCH_SIZE):
        ...
        time.sleep(BATCH_SLEEP)
    if not results:
        fallback_symbols = list(dict.fromkeys(MY_STOCKS + ETF_T_STOCKS))
        for sym in fallback_symbols:
            try:
                r = _score_hot_candidate_fallback(sym)
                if r:
                    results.append(r)
            except Exception:
                pass
...
def score_t_stock(symbol, daily_df, intraday_df=None, market_mode="NEUTRAL"):
    """V23.2 做T评分：严格A级 + 事件风控 + Gap过滤 + 动态热点池 + 双止损。"""
    ...
    # V22/V23.2 事件风险：财报窗口 / 大幅Gap 优先降级
    event_notes = []
...
# ============================================================
# V24 全面提升：市场模式 / 板块热度 / ETF双向T / 每日复盘
# ============================================================

def market_regime_detail():
    """V24: 更清楚的大盘状态。只用于提醒，不自动下单。"""
    qqq = safe_download("QQQ", "6mo", "1d")
    spy = safe_download("SPY", "6mo", "1d")
    if qqq is None or spy is None:
        return {"mode": "NEUTRAL", "score": 0, "note": "QQQ/SPY资料不足", "items": []}

    q = qqq["Close"]; s = spy["Close"]
    q_last = float(q.iloc[-1]); s_last = float(s.iloc[-1])
    q_prev = float(q.iloc[-2]); s_prev = float(s.iloc[-2])
    q_ma20 = float(q.rolling(20).mean().iloc[-1]); q_ma50 = float(q.rolling(50).mean().iloc[-1])
    s_ma20 = float(s.rolling(20).mean().iloc[-1]); s_ma50 = float(s.rolling(50).mean().iloc[-1])
    q_rsi = float(calc_rsi(q).iloc[-1]); s_rsi = float(calc_rsi(s).iloc[-1])
    q_change1 = pct(q_last, q_prev); s_change1 = pct(s_last, s_prev)
    q_change5 = pct(q_last, float(q.iloc[-6])); s_change5 = pct(s_last, float(s.iloc[-6]))

    score = 0
    items = []
    if q_last > q_ma20:
        score += 20; items.append("QQQ站上MA20")
    else:
        score -= 20; items.append("QQQ跌破MA20")
    if s_last > s_ma20:
        score += 15; items.append("SPY站上MA20")
    else:
        score -= 15; items.append("SPY跌破MA20")
    if q_ma20 > q_ma50:
        score += 15; items.append("QQQ中期多头")
    else:
        score -= 10; items.append("QQQ中期转弱")
    if q_change1 > 0 and s_change1 > 0:
        score += 15; items.append("大盘今日同步走强")
    elif q_change1 < -0.8 or s_change1 < -0.8:
        score -= 15; items.append("大盘今日明显回调")
    if q_change5 > 2:
        score += 10; items.append("QQQ 5日强势")
    elif q_change5 < -2:
        score -= 10; items.append("QQQ 5日转弱")
    if q_rsi >= 75:
        score -= 15; items.append("QQQ RSI过热")
    elif 50 <= q_rsi <= 68:
        score += 10; items.append("QQQ RSI健康")

    if score >= 45:
        mode = "RISK_ON"
        note = "进攻盘：可优先看强势股回踩和TQQQ，但仍禁止追高。"
    elif score <= -30:
        mode = "RISK_OFF"
        note = "防守盘：降低仓位，优先保护持仓；可观察SQQQ。"
    else:
        mode = "NEUTRAL"
        note = "震荡盘：只做A/B级机会，仓位降低。"

    return {
        "mode": mode,
        "score": round(score, 1),
        "note": note,
        "qqq": {"price": round(q_last, 2), "change1": round(q_change1, 2), "change5": round(q_change5, 2), "rsi": round(q_rsi, 1), "ma20": round(q_ma20, 2)},
        "spy": {"price": round(s_last, 2), "change1": round(s_change1, 2), "change5": round(s_change5, 2), "rsi": round(s_rsi, 1), "ma20": round(s_ma20, 2)},
        "items": items[:8]
    }


def build_regime_text(detail):
    lines = ["🧭 V24 Market Regime 市场模式", now_kl_str(), ""]
    lines.append(f"模式：{detail.get('mode')}｜分数：{detail.get('score')}")
    lines.append(detail.get("note", ""))
    q = detail.get("qqq", {})
    s = detail.get("spy", {})
    lines.append(f"QQQ：{q.get('price','-')}｜日变动 {q.get('change1','-')}%｜5日 {q.get('change5','-')}%｜RSI {q.get('rsi','-')}｜MA20 {q.get('ma20','-')}")
    lines.append(f"SPY：{s.get('price','-')}｜日变动 {s.get('change1','-')}%｜5日 {s.get('change5','-')}%｜RSI {s.get('rsi','-')}｜MA20 {s.get('ma20','-')}")
    if detail.get("items"):
        lines.append("逻辑：" + " / ".join(detail["items"]))
    lines.append("")
    lines.append("规则：Risk ON可做强势回踩；Neutral只做A/B级；Risk OFF优先防守或观察SQQQ。")
    return "\n".join(lines)
...
def sector_heatmap():
    """V24: 板块热度。用每个板块股票1日/5日平均表现与红盘比例评分。"""
    rows = []
    for sector, symbols in SECTOR_POOLS.items():
        items = []
        for sym in symbols:
            df = safe_download(sym, "3mo", "1d")
            if df is None or len(df) < 8:
                continue
            close = df["Close"]
            last = float(close.iloc[-1])
            ch1 = pct(last, float(close.iloc[-2]))
            ch5 = pct(last, float(close.iloc[-6]))
            items.append({"symbol": sym, "change1": ch1, "change5": ch5})
            time.sleep(0.03)
        if not items:
            continue
        avg1 = sum(x["change1"] for x in items) / len(items)
        avg5 = sum(x["change5"] for x in items) / len(items)
        green_ratio = len([x for x in items if x["change1"] > 0]) / len(items) * 100
        leaders = sorted(items, key=lambda x: (x["change1"], x["change5"]), reverse=True)[:3]
        heat_score = avg1 * 3 + avg5 * 0.8 + green_ratio * 0.08
        rows.append({
            "sector": sector,
            "heat_score": round(heat_score, 1),
            "avg_change1": round(avg1, 2),
            "avg_change5": round(avg5, 2),
...
def build_sector_heat_text(rows):
    lines = ["🔥 V24 Sector Heatmap 板块热度", now_kl_str(), ""]
    if not rows:
        lines.append("暂无板块数据")
        return "\n".join(lines)
    for i, r in enumerate(rows[:8], 1):
        leaders = ", ".join([f"{x['symbol']} {round(x['change1'],2)}%" for x in r.get("leaders", [])])
        lines.append(f"{i}. {r['sector']}｜热度 {r['heat_score']}｜日均 {r['avg_change1']}%｜5日均 {r['avg_change5']}%｜红盘 {r['green_ratio']}%")
        if leaders:
            lines.append(f"Leader：{leaders}")
    lines.append("")
    lines.append("用法：优先从最热板块里找A/B级T机会；弱板块只观察，不硬做。")
    return "\n".join(lines)
...
def run_t_etf(force=False):
    ...
    results = []
    for sym in ETF_T_STOCKS:
        daily = safe_download(sym, "6mo", "1d")
        intraday = safe_download_intraday(sym, "5d", "5m", prepost=True)
        r = score_t_stock(sym, daily, intraday, market_mode="BULLISH" if mode == "RISK_ON" else "DEFENSIVE" if mode == "RISK_OFF" else "NEUTRAL")
        if not r:
            continue
        if sym in ["TQQQ", "SOXL"]:
            r["direction_note"] = "进攻ETF：只适合Risk ON或强反弹日。"
            if mode == "RISK_OFF":
                r["t_score"] = min(r["t_score"], 55)
                r["warnings"].append("Risk OFF，不优先做多杠杆ETF")
        elif sym in ["SQQQ", "SOXS"]:
            r["direction_note"] = "反向ETF：只适合Risk OFF或QQQ/半导体跌破VWAP。"
            if mode == "RISK_ON":
                r["t_score"] = min(r["t_score"], 50)
                r["warnings"].append("Risk ON，不优先做反向ETF")
        else:
            r["direction_note"] = "指数参考，不是主要交易标的。"
        results.append(r)
    results.sort(key=lambda x: (x["t_score"], x.get("effective_rvol", 0)), reverse=True)

    lines = ["⚖️ V24 双向ETF T 雷达", now_kl_str(), ""]
    lines.append(f"大盘模式：{mode}｜{regime.get('note','')}")
    lines.append("")
    if not results:
        lines.append("暂无ETF T数据")
    for i, r in enumerate(results, 1):
        lines.append(f"{i}. {r['symbol']}｜T分 {r['t_score']}｜{r['t_level']}｜{r.get('action_signal','-')}")
        lines.append(f"价 {r['price']}｜VWAP {r.get('vwap','-')}｜离VWAP {r.get('dist_vwap','-')}%｜RVOL {r.get('effective_rvol','-')}")
        lines.append(f"低吸 {r['dip_low']}-{r['dip_high']}｜高抛 {r['sell_low']}-{r['sell_high']}｜止损 {r.get('daytrade_stop','-')}")
        lines.append(f"说明：{r.get('direction_note','')}")
        if r.get("warnings"):
            lines.append("提醒：" + " / ".join(r["warnings"][:4]))
        lines.append("")
    lines.append("规则：ETF只做短T，不长期抱；SQQQ/SOXS是反向杠杆，亏损速度很快。")
    send_telegram("\n".join(lines))
    return {"status": "ok", "regime": regime, "etf": results}


def build_daily_review_text():
    summary = summarize_trades()
    regime = market_regime_detail()
    lines = ["📒 V24 Daily Review 每日复盘", now_kl_str(), ""]
    lines.append(f"市场模式：{regime.get('mode')}｜{regime.get('note')}")
    lines.append(f"交易记录：{summary['total_trades']}笔｜闭合：{summary['closed_rounds']}次｜胜率：{summary['win_rate']}%｜已实现PnL：${summary['realized_pnl']}")
    ...
    lines.append("")
    lines.append("复盘提醒：今天最重要不是交易多，而是只做符合市场模式的A/B级机会。")
    return "\n".join(lines)
...
def build_t_radar_text(title, arr):
    ...
    lines.append("V24做T规则：先看大盘模式/事件/GAP，再看RVOL、VWAP、ORB。A级可重点，B级轻仓，C级观察，D级不做。")
    return "\n".join(lines)
...
def cooldown_check(name, seconds, force=False):
    """V23.2 防重复发送锁：同一个功能在冷却时间内不会重复推送Telegram。"""
    if force:
        return True, 0
    now_ts = time.time()
...
def build_trade_summary_text():
    summary = summarize_trades()
    lines = ["📘 V23.2 交易数据库总结", now_kl_str(), ""]
...
def run_t_radar(force=False):
    ...
    save_state()
    send_telegram(build_t_radar_text("⚡ V23.2 专业做T雷达 Top10", top))
    return {"status": "ok", "top": top, "watchlist": get_t_radar_symbols()}


def run_t_live(force=False):
    """V23.2 盘中确认：使用同一套做T评分，但独立冷却时间，避免盘中刷屏。"""
    ...
    send_telegram(build_t_radar_text("⚡ V23.2 T Live 盘中确认 Top10", top))
    return {"status": "ok", "top": top, "watchlist": get_t_radar_symbols()}
...
def scheduler_loop():
    ...
    for d in weekdays:
        ...
        d.at("05:10").do(run_daily_review)
...
@app.route("/")
def home():
    return "V24 Professional T-Radar + Hot Pool Fix + ETF T + Sector Heat + Daily Review Running"

@app.route("/health")
def health():
    load_watchlist()
    return jsonify({"status": "healthy", "time_kl": now_kl_str(), "symbols": len(ALL_SYMBOLS), "my_stocks": MY_STOCKS, "positions_count": len(load_positions()), "version": "V24_hotpool_fix_etf_sector_review"})
...
@app.route("/run-regime")
def route_regime_run():
    force = request.args.get("force", "1") == "1"
    return jsonify(run_regime(force=force))

@app.route("/run-sector-heat")
def route_sector_heat_run():
    force = request.args.get("force", "1") == "1"
    return jsonify(run_sector_heat(force=force))

@app.route("/run-t-etf")
def route_t_etf_run():
    force = request.args.get("force", "1") == "1"
    return jsonify(run_t_etf(force=force))

@app.route("/run-daily-review")
...
def route_t_radar_info():
    return jsonify({
        ...
        "usage_regime": "/run-regime",
        "usage_sector_heat": "/run-sector-heat",
        "usage_t_etf": "/run-t-etf",
        "usage_daily_review": "/run-daily-review",
        ...
        "usage_set_watchlist": "/watchlist/set?symbols=LUNR,IONQ,SMCI,AMD,CRM,TSLA,META,SOFI,PLTR,TEM,AMZN,APP,ONDS,SOUN,NVDA,AFRM",
        "env_optional": "T_RADAR_STOCKS=LUNR,IONQ,SMCI,AMD,CRM,TSLA,META,SOFI,PLTR,TEM,AMZN,APP,ONDS,SOUN,NVDA,AFRM"
    })
...
@app.route("/bad-symbols")
def route_bad_symbols():
    active = {}
    now_ts = time.time()
    for sym, item in YF_FAIL_CACHE.items():
        remain = int((YF_FAIL_SKIP_MINUTES * 60) - (now_ts - float(item.get("last_ts", 0))))
        if remain > 0:
            active[sym] = {
                "fail_count": item.get("count", 0),
                "reason": item.get("reason", ""),
                "cooldown_remaining_sec": remain
            }
    return jsonify({"count": len(active), "bad_symbols_runtime": active})


@app.route("/bad-symbols/clear")
def route_bad_symbols_clear():
    YF_FAIL_CACHE.clear()
    return jsonify({"status": "ok", "message": "runtime bad symbol cache cleared"})


@app.route("/system/status")
def route_system_status():
    return jsonify({
        "version": "V24_stable_professional_t_radar",
        "time_kl": now_kl_str(),
        "hot_pool_scan_limit": HOT_POOL_SCAN_LIMIT,
        "yf_suppress_errors": YF_SUPPRESS_ERRORS,
        "yf_fail_limit": YF_FAIL_LIMIT,
        "yf_fail_skip_minutes": YF_FAIL_SKIP_MINUTES,
        "cooldown": {
...
def route_test():
    ok = send_telegram("✅ V23.2 Professional T-Radar Telegram Test Success")
